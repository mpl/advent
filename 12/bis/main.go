...
func tryAllAs() position {
	// TODO: do better
	lowestWeight := 500
	var bestA position
	for k, v := range topomap {
		if v != bottom {
			continue
		}
		w := weights[k]
		// If need be, also check in seen?
		if w == 0 {
			// a in a hole
			continue
		}
		// println("FOUND", k.x, k.y, w)
		if w < lowestWeight {
			lowestWeight = w
			bestA = k
		}
	}
	println("LOWEST WEIGHT: ", lowestWeight)
	return bestA
}
